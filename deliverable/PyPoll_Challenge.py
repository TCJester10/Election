...
file_to_save = os.path.join("..","deliverable", "election_analysis.txt")
...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if county_name not in county_options:
            ...
            #print("RIGHT HERE ITS ME")
            #print(f"county_name {county_name} county_options {county_options} county_votes {county_votes}")
...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        county_results = (
            f"{county_name}: {cvote_percentage:.1f}% ({cv:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (cv > Largest_county_votes):
            Largest_county = county_name
            Largest_county_votes = cv
